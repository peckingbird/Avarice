  ...
  public List<Player> getPlayersForGame(String gameId) {
    ...
    return gamePlayers.stream()
        .map(
            gamePlayer ->
                playerRepository
                    .findById(gamePlayer.playerId())
                    .orElseThrow(
                        () ->
                            new InvalidStateException(
                                String.format(
                                    "Player not found with id: %s", gamePlayer.playerId()))))
        .toList();
  }
  ...
  public void addPlayerToGame(String playerId, String gameId) {
    ...
    Game game =
        gameRepository
            ...
            .orElseThrow(
                () ->
                    new InvalidStateException(String.format("Game not found with id: %s", gameId)));

    Optional<GamePlayer> gamePlayer =
        gamePlayerRepository.findByGameIdAndPlayerId(game.gameId(), player.playerId());

    if (gamePlayer.isEmpty()) {
      gamePlayerRepository.save(new GamePlayer(null, game.gameId(), player.playerId()));
    }
  }
}
